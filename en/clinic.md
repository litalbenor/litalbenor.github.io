---
layout: default
title: The Clinic
lang: en
dir: ltr
permalink: /en/clinic
---

<div class="container container--narrow">
  <header class="page-header">
    <h1 class="page-title">The Clinic</h1>
    <p class="page-intro">A calm, private space designed for your comfort and confidentiality.</p>
  </header>

  <div class="page-content">
    <div class="clinic-image">
      [Clinic photo placeholder]
    </div>

    <section class="content-section">
      <h2>A Welcoming Space</h2>
      <p>
        My clinic is located in a quiet, residential area of North London (N12), offering a peaceful environment away from the bustle of central London. The space is designed to feel warm and comfortable, helping you feel at ease from the moment you arrive.
      </p>
      <p>
        Privacy and confidentiality are paramount. The clinic has a private entrance and waiting area, ensuring your visits remain discreet.
      </p>
    </section>

    <section class="content-section">
      <h2>Location</h2>
      <div class="location-box">
        <p>
          <strong>Address:</strong> <a href="https://maps.google.com/?q=34+Gainsborough+Road,+London+N12+8AG" target="_blank" rel="noopener">34D Gainsborough Road, London N12 8AG, United Kingdom</a>
        </p>
      </div>
    </section>

    <section class="content-section">
      <h2>Getting Here</h2>
      
      <h3>By Underground</h3>
      <p>
        <strong>Woodside Park Station</strong> (Northern Line) — The clinic is a short walk from the station.
      </p>
      
      <h3>By Bus</h3>
      <p>
        <strong>North Finchley Bus Station</strong> — Multiple bus routes serve this area, making it easily accessible from across North London.
      </p>
      
      <h3>By Car</h3>
      <p>
        Free street parking is available in the surrounding residential streets. [Additional parking information to be added]
      </p>
    </section>

    <section class="content-section">
      <h2>Accessibility</h2>
      <p>
        [Placeholder: Information about wheelchair access, step-free entry, and any other accessibility features]
      </p>
    </section>

    <section class="content-section">
      <h2>Online Sessions</h2>
      <p>
        If you prefer to meet online or are unable to travel to the clinic, I offer secure video sessions. Online therapy provides the same quality of care and confidentiality as in-person sessions.
      </p>
    </section>
  </div>
</div>
